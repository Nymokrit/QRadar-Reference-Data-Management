...
function Sidebar(props) {
    ...
    return (
        <div className='menu' >
            <Search type='text' labelText={i18n.t('data.sidebar.search')} placeHolderText={i18n.t('data.sidebar.search')} light onChange={
                searchInputChanged} value={searchText} />
            <Accordion>
                {
                    Object.keys(props.refData).map(type =>
                        <AccordionItem key={type} open={open[type]} title={props.refData[type].label} className='menu-title' onClick={() => toggleMenu(type)}>
                            <Button kind='primary' size='small' className='btn-create-new' onClick={(e) => props.createEntry(e, type)} key={'btn-' + type}>
